...
module.exports = function generator({
  ...
  if (typeKind === 'SCALAR') {
    ...
    // Time
    if (typeName === 'Time') {
      const ex = nameIncludes(thingName, 'expir', 'schedule', 'start', 'end', 'due', 'target', 'future')
        ? futureDate()
        : pastDate()
    ...
    // Boolean
    if (typeName === 'Boolean') {
      const falseNames = ['disabled', 'hidden', 'deleted', 'inactive', 'failed', 'rejected', 'voided', 'silent', 'static', 'isolated']
      const ex = falseNames.some((n) => nameIncludes(thingName, n)) ? false : true
      return isArray ? [ex] : ex
    }
    ...
    if (typeName === 'Float') {
      ...
      if (nameIncludes(thingName, 'lon', 'lng')) return isArray ? [3.4219] : 3.4219
      if (nameIncludes(thingName, 'rate', 'percent', 'factor', 'ratio')) return isArray ? [1.5] : 1.5
      if (nameIncludes(thingName, 'amount', 'price', 'total', 'fee', 'balance')) return isArray ? [250000.0] : 250000.0
      if (nameIncludes(thingName, 'quantity', 'count', 'weight')) return isArray ? [2.0] : 2.0
